describe("Request Group Session, Accepting the session & chatting", () => {
    it("should successfully handle the group session request, accept it and chat", () => {
        ...
        cy.visit("http://127.0.0.1:8000/groupSession");
        ...
        cy.contains("Log out").should("be.visible").click();
        cy.visit("http://127.0.0.1:8000/login");
        cy.get('input[name="email"]').type("thejaka.fdo@gmail.com");
        cy.get('input[name="password"]').type("password");
        cy.get('button[type="submit"]').click();

        // Step 29: Navigate to "My Sessions"
        cy.visit("http://127.0.0.1:8000/my-requests");

        // Step 30: Locate the session card and click "Chat Now"
        cy.contains('a', "Chat Now").click();

        // Step 31: Send a message in the chat
        cy.get('textarea[wire\\:model="newMessage"]')
            .type("Hey guys, can we have it around 10.30 AM? I'm not sure if I'll be able to reach at that time...");
        cy.get('button[type="submit"]').contains("Send").click();
    });
}); 